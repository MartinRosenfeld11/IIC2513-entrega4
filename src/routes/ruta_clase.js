...
router.post('clase.create', '/', async(ctx) => {
    try{
        const clase = await ctx.orm.Clase.create(ctx.request.body)
        ctx.body = clase;
        ctx.status = 201;
    } catch(error) {
        ctx.body = error;
...
router.get("clase.list", '/claseData', async(ctx) => {
    try {
        const clase = await ctx.orm.Clase.findAll()
        ctx.body = clase;
        ctx.status = 200;
    } catch(error) {
        ctx.body = error;
...
router.get('clase.show','/:id', async(ctx) => {
    try{
        const clase = await ctx.orm.Clase.findOne({where:{id: ctx.params.id}});
        ctx.body = clase;
        ctx.status = 201;
    } catch(error) {
        ctx.body = error;
...
router.delete('clase.delete', '/:id', async (ctx) => {
    try {
        const clase = await ctx.orm.Clase.findByPk(ctx.params.id);
